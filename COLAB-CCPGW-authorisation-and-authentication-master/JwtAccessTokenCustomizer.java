/**
 * JwtAccessTokenCustomizer.java — Class that takes care extracting user_name 
 * and roles from access token and setting it to spring security context.
 * 
 * In order for Spring OAuth2 to parse and set SecurityConextHolder, it needs 
 * the roles or authorities from token. Also, in order to determine the list of 
 * clients/application/micro-service a user has access, it needs the list of 
 * client ids from token. This is the only setup that needs some special handling
 */
